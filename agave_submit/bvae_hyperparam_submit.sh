...
#SBATCH -o slurm_VAE.%j.out          # STDOUT (%j = JobId)  # Outfile
#SBATCH -e slurm.%j.err          # STDERR (%j = JobId)   # Errorfile
#SBATCH -p sulcgpu1    # Partition of ASU HPC Agave cluster to use
#SBATCH -q sulcgpu1      # The queue to submit to, vary by partition
#SBATCH --gres=gpu:2     # Request 2 GPU to run this job
...
python bvae_hparam_opt.py      # Run our script
